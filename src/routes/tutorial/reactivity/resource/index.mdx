...
Qwik предоставляет [`useResource$()`](/docs/(qwik)/components/resource/index.mdx) и `<Resource>`, чтобы помочь вам получить и отрендерить данные с сервера. При получении данных приложение может находиться в одном из трёх состояний:
...
Используйте функцию [`useResource$()`](/docs/(qwik)/components/resource/index.mdx), чтобы настроить способ получения данных с сервера. Используйте `<Resource>` для отображения данных.

## Получение данных

Используйте [`useResource$()`](/docs/(qwik)/components/resource/index.mdx), чтобы настроить способ получения данных с сервера.
...
Функция [`useResource$()`](/docs/(qwik)/components/resource/index.mdx) возвращает объект `ResourceReturn`, который представляет собой объект, подобный промисам, и может быть сериализован Qwik. Функция [`useResource$()`](/docs/(qwik)/components/resource/index.mdx) позволяет с помощью `track` отслеживать свойства для того, чтобы функция [`useResource$()`](/docs/(qwik)/components/resource/index.mdx) могла реагировать на изменения состояния. Функция `cleanup` позволяет зарегистрировать код, который необходимо выполнить, чтобы освободить ресурсы от предыдущего запроса. Наконец, функция [`useResource$()`](/docs/(qwik)/components/resource/index.mdx) возвращает промис, который разрешается в значение.

## Рендер данных

Используйте `<Resource>` для отображения данных функции [`useResource$()`](/docs/(qwik)/components/resource/index.mdx). `<Resource>` позволяет вам отображать различное содержимое в зависимости от текущего состояния ресурса: `pending`, `resolved` или `rejected`.
