...
> Если у вас ещё нет приложения Qwik, то вам нужно [сначала создать его](/docs/(qwik)/getting-started/index.mdx), затем, следуя инструкциям, выполнить команду добавления React к вашему приложению.
...
> **Примечание:** Вы НЕ МОЖЕТЕ использовать компоненты React в Qwik без их предварительного преобразования с помощью `qwikify$()`. Несмотря на то, что [компоненты React и Qwik выглядят похоже](/docs/(qwikcity)/guides/react-cheat-sheet/index.mdx), они принципиально очень разные.
