...
Использовать `supabase` в Qwik очень просто. Библиотека Supabase JS прекрасно взаимодействует с серверными API Qwik, такими как `routeLoader$`, `routeAction` или `server$`.
