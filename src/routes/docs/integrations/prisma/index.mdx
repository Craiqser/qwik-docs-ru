...
Prisma OCR можно использовать в Qwik с помощью функций `routeLoader$`, `routeAction$` и `server$`. Это API, позволяющие выполнять код только на стороне сервера, что и нужно для Prisma.
