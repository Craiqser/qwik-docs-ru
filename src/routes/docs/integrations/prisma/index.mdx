...
В Prisma вы определяете схему БД в файлах `.prisma`, а CLI автоматически генерирует миграции БД, а также типы Typescript.

Prima ORM можно использовать в Qwik с функциями `routeLoader$`, `routeAction$` и `server$`. Это API Qwik, позволяющие выполнять код только на стороне сервера.
