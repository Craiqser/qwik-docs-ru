...
Prisma Client требует двоичный файл WASM размером 14 МБ, который может оказаться слишком большим, если вы планируете развернуть своё приложение в бессерверной среде. Мы рекомендуем использовать Prisma только при развертывании в среде Node.js, такой как Google Cloud Run, Express или Azure.
