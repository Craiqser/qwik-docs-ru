...
Prisma ORM можно использовать в Qwik с помощью функций `routeLoader$`, `routeAction$` и `server$`. Эти API позволяют выполнять код только на стороне сервера, что и нужно Prisma.
