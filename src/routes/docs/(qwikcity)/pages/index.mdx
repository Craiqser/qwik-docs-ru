...
Вы также можете экспортировать функцию, которая возвращает объект `DocumentHead`, позволяя программно установить заголовок или мета-свойство.

Это позволяет настроить `<head>`, включая заголовок, используя данные из `routeLoader$()` или `routeAction$()`.
