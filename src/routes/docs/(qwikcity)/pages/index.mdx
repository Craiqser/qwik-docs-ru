...
export const head: DocumentHead = {
  // Это будет использоваться для определения элемента <title> страницы
  title: 'About page',
  meta: [
    {
      name: 'description',
...
Вы также можете экспортировать функцию, которая возвращает объект `DocumentHead`. Это позволяет динамически устанавливать заголовок страницы на основе данных, которые извлекаются из `routeLoader$()` или `routeAction$()`, выполняемых перед отрисовкой компонента страницы.
