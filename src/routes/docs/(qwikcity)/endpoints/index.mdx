...
Конечные точки полезны, когда вам нужно открыть какой-либо внешний API, например, RESTful API или GraphQL API, для использования внешним субъектом, такому как мобильное приложение или сторонний сервис.
...
Маршруты конечных точек открывают соответствующие конечные точки HTTP, к которым можно получить доступ извне с любого клиента, в то время как функции `server$` доступны только внутри проекта Qwik.
...
Конечные точки могут перенаправлять на другой URL, используя метод `redirect(status, url)`. Метод `redirect()` автоматически установит заголовок `Location` на URL перенаправления.
