...
Действия похожи на [загрузчики](/qwikcity/loader/), но они не выполняются во время рендера. Вместо этого они выполняются при явном вызове, то есть запускаются после некоторого взаимодействия с пользователем, например, при нажатии кнопки мыши или при отправке формы.
