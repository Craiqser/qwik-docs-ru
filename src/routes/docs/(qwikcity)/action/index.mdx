...
Действия позволяют обрабатывать отправленные формы на стороне сервера, позволяя выполнять побочные эффекты, такие как запись в базу данных или отправка электронного письма. Они также могут возвращать данные клиенту, позволяя соответствующим образом обновлять пользовательский интерфейс.
  ...
  return (
    <>
      ...
      // Если действие выполнено успешно, свойство `action.value` будет содержать возвращаемое значение действия.
        <div>Пользователь {action.value.userID} успешно добавлен</div>
      )}
    </>
  );
});
...
Qwik поставляется со встроенной поддержкой [Zod](https://zod.dev/), которую можно использовать непосредственно с действиями, используя функцию `zod$()`.
...
export const useAddUser = routeAction$(
  async (user) => {
    // `user` типизирован как { firstName: string, lastName: string }
    const userID = await db.users.add({
      firstName: data.firstName,
      lastName: data.lastName,
    });
