...
К счастью, действия имеют первоклассную поддержку библиотеки [Zod](https://zod.dev/) для проверки данных на основе TypeScript. Чтобы использовать Zod, просто передайте схему Zod в качестве второго аргумента функции `routeAction$()`.
...
При передаче данных в `routeAction()` они проверяются на соответствие схеме Zod. Если данные недействительны, действие поместит ошибку валидации в свойство `routeAction.fail`.
