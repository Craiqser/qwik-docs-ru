...
export const getProductData = routeLoader$(async () => {
  // Добавьте сюда код получения данных (fetch, DB, Firebase и т.д.).
  // const res = await fetch('https://.../product')
  // const products = await res.json()
  // return products;

  // Мы в этом примере просто возвращаем данные.
  return {
    product: {
      name: 'Qwik City',
      price: 100,
...
Приведённый выше пример показывает использование `useGetServerTime()` в двух разных компонентах и в разных файлах. Это намеренное поведение.

**Файл: `src/routes/admin/index.tsx`**
