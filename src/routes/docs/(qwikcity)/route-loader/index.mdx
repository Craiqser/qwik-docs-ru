...
> Вы не должны использовать `routeLoader$` для создания REST API, для этого лучше использовать [Endpoint](/docs/(qwikcity)/middleware/index.mdx), который позволяет жёстко контролировать заголовки и тело ответа.
...
Подобно [промежуточному ПО](/docs/middleware/) или [конечной точке](/docs/endpoint), `onRequest` и `onGet`, `routeLoader$` имеет доступ к API [`RequestEvent`](/docs/middleware#requestevent), который содержит информацию о текущем HTTP-запросе.
