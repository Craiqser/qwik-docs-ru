...
Загрузчики маршрутов загружают данные на сервер, чтобы они стали доступны для использования в компонентах Qwik. Они ведут себя как функции RPC на стороне сервера, которые могут быть вызваны компонентами Qwik во время рендера.
...
> Вы не должны использовать `routeLoader$` для создания REST API, для этого лучше использовать [Endpoint](/docs/(qwikcity)/endpoints/index.mdx), который позволяет жёстко контролировать заголовки и тело ответа.
...
Подобно обработчикам запросов, таким как `onRequest` и `onGet`, `routeLoader$` имеет доступ к объекту `RequestEvent`, который содержит информацию о текущем HTTP-запросе и ответе.
...
Загрузчики выполняются после обработчиков маршрутов Qwik (`onRequest`, `onGet`, `onPost` и т.д.), и перед рендером компонентов Qwik, что позволяет начать получение данных как можно быстрее, уменьшая задержку.
