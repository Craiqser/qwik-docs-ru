...
Он также принимает необязательный параметр `restoreScroll$`, который может быть использован для настройки поведения прокрутки в SPA-приложении. Функция, переданная в `restoreScroll$`, вызывается со следующими аргументами по событиям навигации:
...
- `toTopAlways`, как следует из названия, всегда прокручивает страницу до верхней части окна при переходе к новым или предыдущим страницам.
- `toLastPositionOnPopState` (по умолчанию) имитирует работу браузеров с MPA - прокрутка страницы к вершине при переходе на новую страницу, но к последней посещенной позиции при переходе по событию `popstate` (например, кнопки назад/далее).

> Обратите внимание, что такие браузеры, как Chromium и Firefox, изначально прокручивают до хэшей при обычной навигации, а всплывающие состояния ВСЕГДА восстанавливаются. Предустановка `toLastPositionOnPopState` будет повторять это поведение, в то время как `toTopAlways` всегда будет прокручивать до хэшей.

Вы также можете реализовать свою собственную логику восстановления прокрутки, предоставив функцию с приведённой выше сигнатурой, например:

```ts
const restoreScroll: QRL<RestoreScroll> = $((type, fromUrl, toUrl, scrollState) => {
  // Делаем что-то перед навигацией:
  prepare();
...
> Обратите внимание, что возвращаемая внутренняя функция void ДОЛЖНА выполняться синхронно с рендером, рекомендуется избегать внутри неё дорогостоящих вычислений.

> Аргумент `scrollState` может быть неопределённым, когда нет истории прокрутки, обычно, когда тип не является popstate, но также всплывает при пользовательском `pushState`.

## `<RouterOutlet>`

Компонент `RouterOutlet` отвечает за рендер совпадающего в данный момент маршрута, он использует под капотом [`useContent()`](#usecontent) и отвечает за отрисовку текущей страницы, а также всех вложенных макетов.
