...
Функция `useDocumentHead()` используется для чтения метаданных [заголовка документа](/docs/(qwikcity)/pages/index.mdx#head-export).
...
Компонент `RouterOutlet` отвечает за рендер сопоставленного маршрута, используя внутреннюю функцию [`useContent()`](/docs/(qwikcity)/api/index.mdx#usecontent), и для рендера текущей страницы, а также всех вложенных макетов.

Этот компонент обычно располагается как дочерний компонент `<body>`, в большинстве стартовых шаблонов вы найдете его в файле `src/root.tsx` (см. пример в `QwikCityProvider`).

Он принимает необязательный параметр `restoreScroll$`, который может быть использован для настройки поведения прокрутки в SPA-приложении. Функция, переданная в `restoreScroll$`, вызывается со следующими аргументами по событиям навигации:
...
В QwikCity есть две встроенные предустановки восстановления прокрутки - `toTopAlways` и `toLastPositionOnPopState`.

- `toTopAlways` (по умолчанию), как следует из названия, всегда прокручивает страницу до верхней части окна при переходе к новым или предыдущим страницам.
- `toLastPositionOnPopState` имитирует работу браузеров с MPA - прокрутка страницы к вершине при переходе на новую страницу, но к последней посещенной позиции при переходе по событию `popstate` (например, кнопки назад/далее).

> Обратите внимание, что обе предустановки рассматривают хэш-навигацию как исключение и всегда прокручивают страницу к хэшированному элементу, если он доступен и указан в URL.

Вы также можете реализовать собственную логику восстановления состояния прокрутки, предоставив функцию с приведенной выше сигнатурой. Промис `toUrlSettled` позволяет дождаться завершения обновления DOM перед установкой позиции прокрутки.

```ts
const restoreScroll: RestoreScroll = $(async (type, fromUrl, toUrlSettled) => {
  // сделать что-то перед навигацией, например, сохранение текущей позиции прокрутки в `sessionStorage`
  prepare(type, fromUrl);
  // ожидание окончания резолва URL и установки DOM
  const toUrl = await toUrlSettled;
  // Фактическое восстановление прокрутки.
...
export default component$(() => {
  /**
   * Корневой элемент QwikCity-сайта всегда начинается с компонента `<QwikCityProvider>`,
   * сразу за которым следуют `<head>` и `<body>` документа.
   *
...
Компонент `Form` является обёрткой нативного элемента `<form>` и предназначен для работы с [серверными действиями](/docs/(qwikcity)/action/index.mdx#using-actions-with-form).
