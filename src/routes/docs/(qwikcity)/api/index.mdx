...
Эта функция возвращает функцию `nav()`, которая может быть использована для перехода к новому пути.
...
Функция `routeLoader$()` используется для объявления нового серверного загрузчика для заданной страницы/конечной точки или макета. Qwik City выполнит все объявленные загрузчики для заданного маршрута. Компоненты Qwik могут позже ссылаться на загрузчики, импортируя их и вызывая метод `.use()` для получения данных.
