...
Компонент `RouterOutlet` отвечает за рендер сопоставленного маршрута, используя внутреннюю функцию [`useContent()`](#usecontent), и для рендера текущей страницы, а также всех вложенных макетов.
