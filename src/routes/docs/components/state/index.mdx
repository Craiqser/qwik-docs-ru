...
В Qwik есть два способа создания вычисляемых свойств: с помощью [`useTask$()`](./../lifecycle/index.mdx#usewatch) или [`useResource$()`](./../resource/index.mdx).

Основное различие между ними заключается в том, что [`useTask$()`](./../lifecycle/index.mdx#usewatch) допускает побочные эффекты и выполнение сериализуется, а [`useResource$()`](./../resource/index.mdx) является асинхронным и несколько вызовов `useResource$()` могут происходить параллельно.
...
### Пример использования [`useTask$()`](./../lifecycle/index.mdx#usewatch)
