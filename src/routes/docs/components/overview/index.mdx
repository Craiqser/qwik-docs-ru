...
- [`useStore(initialState)`](../state/index.mdx#usestore) - создает реактивный объект, который может быть использован для хранения состояния
- [`useSignal$(initialState)`](../state/index.mdx#usesignal) - создает реактивное значение
- [`createContext(contentName)`](../context/index.mdx#createcontext) - создает ссылку контекста
...
- [`useClientEffect$()`](../lifecycle/index.mdx#useclienteffect) - определяет функцию обратного вызова, которая будет выполняться после рендера, только на клиенте (в браузере).
