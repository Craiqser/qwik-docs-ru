...
- [`useBrowserVisibleTask$()`](../lifecycle/index.mdx#usebrowservisibletask) - определяет функцию обратного вызова, которая будет выполняться после рендера, только на клиенте (в браузере).
