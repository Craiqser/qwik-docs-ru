...
export default component$(() => {
  ...
  return (
    <>
      <h1 class={themeStore.theme}>
        {themeStore.theme === 'dark' ? 'Добро пожаловать на тёмную сторону' : 'Компонент в светлой теме'}
      </h1>
      <button
        onClick$={() => {
          themeStore.theme == 'light' ? (themeStore.theme = 'dark') : (themeStore.theme = 'light');
        }}
      >
        Изменить тему
      </button>
...
- Как и все [`use-методы`](/docs/components/lifecycle/index.mdx#use-method-rules), он используется в корне [`component$()`](/docs/components/overview/index.mdx#component).
...
- `data`: вы можете предоставить любой тип данных, например useStore из Qwik, Array, Objects.
