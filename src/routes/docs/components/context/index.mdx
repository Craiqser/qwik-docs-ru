...
Обратите внимание, что значение, возвращаемое `createContextId()`, не содержит никакого состояния, и оно неизменяемо. Оно используется только для описания имени и типа контекста, как ссылка или идентификатор.
...
Когда контекст создается с помощью `createContextId()`, ему можно указать тип. На самом деле, это настоятельно рекомендуется делать, чтобы уменьшить количество ошибок и опечаток.
