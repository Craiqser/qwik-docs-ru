...
Функция обратного вызова, переданная в [`useResource$()`](./index.mdx), запускается сразу после завершения обратных вызовов `useMount$()` и [`useWatch$()`](./../lifecycle/index.mdx#usewatch). Дополнительные сведения смотри в разделе [Жизненный цикл](./../lifecycle/index.mdx).
