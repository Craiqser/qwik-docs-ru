...
Пример, показывающий, как `useResource$` используется для выполнения запроса данных о погоде при изменении названия города.
...
> **ПРИМЕЧАНИЕ**
> Мы настоятельно рекомендуем использовать `<Resource />` всегда, когда это возможно, поскольку это позволит избежать повторного рендера компонента, а также оптимизировано для SSR.
