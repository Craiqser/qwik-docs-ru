  ...
  return (
    <div>
      ...
      <Resource
        ...
        onResolved={(ageGuess) => {
          return <div>
            {store.name && <>{ageGuess.name} {ageGuess.age} лет</>}
          </div>;
        }}
      />
    </div>
...
Компонент `<Resource />` оптимизирован в плане избегания ненужных ререндеров, а также для SSR. Мы настоятельно рекомендуем использовать `<Resource />` везде, где это возможно. Но вы можете столкнуться с исключительным случаем, когда вам необходимо больше контроля над процессом. В таких ситуациях Qwik позволяет вам использовать данные, возвращаемые из `useResource()` непосредственно в вашем коде TSX, как показано ниже:
  ...
  return (
    <div>
      ...
      <div>
        ...
        {!ageResource.loading && store.name && <div>{ageResource.promise.then(ageGuess => (
          <div>{ageGuess.name} {ageGuess.age} лет</div>
        ))}</div>}
      </div>
    </div>
  );
