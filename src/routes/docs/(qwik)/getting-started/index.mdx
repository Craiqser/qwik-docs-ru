...
2. Затем добавьте хук `useDadJoke()` к компоненту `default` и используйте результат в JSX:
...
<details>
  <summary>Файл: `src/routes/joke/index.tsx`:</summary>
```tsx
import { component$ } from "@builder.io/qwik";
import { routeLoader$ } from "@builder.io/qwik-city";
...
ПРИМЕЧАНИЕ:
- Нажатие на кнопку приведет к обновлению состояния, и пользовательский интерфейс будет обновлен.
...
## Задачи и вызов серверного кода

В Qwik [задача](/docs/components/lifecycle/#use-usetask-when-you-need-to) - это работа, которая должна произойти при изменении состояния (аналогично хуку "effect" в других фреймворках). В этом примере мы будем использовать задачу для вызова кода на сервере.
...
3. Добавьте код, которую вы хотите выполнить при изменении состояния:
...
4. Если вы хотите, чтобы работа происходила только на сервере, оберните ее в `server$()`.
...
- Во время SSR сервер печатает `FAVORITE (isomorphic) false` и `FAVORITE (server) false`;
- Когда пользователь взаимодействует с избранным, клиент печатает `FAVORITE (isomorphic) true`, а сервер печатает `FAVORITE (server) false`.
...
<details>
  <summary>Файл: `src/routes/joke/index.css`:</summary>
```css
div {
  font-weight: bold;
}
...
- [Структура проекта](/docs/project-structure/)
- [Маршрутизация на основе каталогов](/docs/routing/)
- [Компонент](/docs/(qwik)/components/overview/)
- [Загрузчики маршрута](/docs/route-loader/)
- [Действия формы](/docs/action/) (и [валидация с помощью zod](/docs/action/#zod-validation))
- [Управление состоянием](/docs/components/state/)
- [Задачи](/docs/components/lifecycle/#use-usetask-when-you-need-to)
- [Кеш сервис-воркера](/docs/advanced/speculative-module-fetching/)
