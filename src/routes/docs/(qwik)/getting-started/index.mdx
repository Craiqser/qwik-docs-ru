...
Учебник Qwik Hello World поможет вам создать с помощью Qwik шуточное приложение, охватывая при этом наиболее важные концепции Qwik. Приложение отображает случайную шутку с сайта https://icanhazdadjoke.com и содержит кнопку для получения новой шутки по клику.
...
Чтобы загрузить данные для отображения пользователю, используйте [загрузчики маршрута](/docs/route-loader/).
...
3. Теперь по адресу `http://127.0.0.1:5173/` браузер показывает случайную шутку.
...
1. Добавьте этот код в файл `src/routes/joke/index.tsx`:
