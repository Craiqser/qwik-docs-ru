...
<CodeSandbox src={UseTask} sandboxStyle={{ height: '6em' }} />
...
export default component$(() => {
  ...
  useTask$(({ track }) => {
    ...
    isServer
      ? update() // Не делаем задержку на сервере во время SSR.
...
<CodeSandbox src={Track} sandboxStyle={{ height: '6em' }} />
...
<CodeSandbox src={TrackFn} sandboxStyle={{ height: '6em' }} />
...
export default component$(() => {
  ...
  return (
    <div>
      Введите текст: <input bind:value={text} />
      <div>Текст с задержкой: {debounceText}</div>
    </div>
  );
});
```

<CodeSandbox src={Cleanup} sandboxStyle={{ height: '6em' }} />
...
export default component$(() => {
  ...
  useTask$(({ track }) => {
    track(text);
    if (isServer) {
      return; // Защита от запуска на сервере.
    }
    isBold.value = true;
...
const delay = (time: number) => new Promise((resolve) => setTimeout(resolve, time));
```
> <CodeSandbox src={TrackServerGuard} sandboxStyle={{height: '6em'}}/>
  ...
  return (
    <>
      <div style="position: sticky; top:0">
        Scroll to see clock. (Currently clock is {isClockRunning.value ? '' : 'not'} running.)
      </div>
...
<CodeSandbox src={UseVisibleTask} sandboxStyle={{ height: '6em' }} />
...
Иногда желательно запускать `useVisibleTask$()` сразу, как только приложение загружается в браузер. В этом случае `useVisibleTask$()` должна выполняться в eagre-режиме. Это делается с помощью `{ strategy: 'document-ready' }`.
...
export default component$(() => {
  ...
  return (
    <>
      <div style="position: sticky; top:0">
        Прокрутите, чтобы увидеть часы (в настоящее время часы {isClockRunning.value ? '' : 'не'} запущены.)
      </div>
      <div style="height: 200vh" />
      <Clock isRunning={isClockRunning} />
    </>
...
<CodeSandbox src={Resource} sandboxStyle={{height: '6em'}}/>
