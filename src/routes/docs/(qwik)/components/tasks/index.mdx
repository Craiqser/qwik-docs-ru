...
В Qwik не существует конкретного хука "on-mount", потому что `useTask$()` без отслеживания эффективно ведёт себя как хук `mount`.

Это связано с тем, что `useTask$` всегда запускается хотя бы раз при первом монтировании компонента.
