...
В приведённом выше примере мы создаем `ContextId` с именем `docs.theme-context` и затем используем его для предоставления `useSignal` компоненту `Parent`. Компонент `Child` затем использует метод `useContext` для получения `useSignal` и рендера значения.
