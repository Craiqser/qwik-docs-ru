...
export default component$(() => {
  ...
  return (
    <div>
      ...
      >
        Измените текст, чтобы растянуть поле.
      </div>
      <div>
        Приведённое выше красное поле имеет высоту {height.value} пикселей и ширину {width.value} пикселей.
      </div>
    </div>
  );
});
...
- [`useResource$()`](../tasks/index.mdx#useresource) - создаёт ресурс для асинхронной загрузки данных;
