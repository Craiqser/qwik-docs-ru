...
2. [`useResource$()`](/docs/(qwik)/components/state/index.mdx): `useResource$()` используется, когда вычисляемое свойство является асинхронным или состояние приходит извне приложения. Например, получение текущей погоды (внешнее состояние) на основе текущего местоположения (внутреннее состояние приложения).

Помимо двух описанных выше способов создания вычисляемых свойств, существует также более низкоуровневый ([`useTask$()`](/docs/(qwik)/components/tasks/index.mdx#usetask). Этот способ не производит новый сигнал, а скорее изменяет существующее состояние или производит побочный эффект.
...
Используйте `useResource$()` для создания вычисляемого значения, которое выводится асинхронно или требует внешнего состояния (например, обращения к конечной точке REST). Хук вызывается при установке компонента (и при изменении отслеживаемых значений). Хук `useResource$` предназначен для использования с `<Resource />`. Компонент `<Resource />` - это удобный способ отображения состояния ресурса, таких как: загрузка, промис разрешён, промис отклонён.

> **Примечание:** Важно понимать, что `useResource$` выполняется при начальном рендере компонента (так же, как `useTask$`). Часто бывает желательно начать выборку данных на сервере в рамках первоначального HTTP-запроса до того, как компонент будет отображён. Получение данных как часть SSR является более распространенным и предпочтительным способом загрузки данных и выполняется через API [`routeLoader$`](/docs/(qwikcity)/route-loader/index.mdx). `useResource$` - это скорее низкоуровневый API, который полезен, когда вы хотите получить данные в браузере.
>
> Как и все хуки `use-`, он должен быть вызван в контексте [`component$()`](/docs/(qwik)/components/overview/index.mdx#component), при этом применяются все [правила хуков](/docs/(qwik)/components/tasks/index.mdx).
>
> Во многих отношениях `useResource$` похож на `useTask$`. Большие различия заключаются в следующем:
>
> - `useResource$` позволяет вам возвращать "значение";
> - `useResource$` не блокирует рендер, пока ресурс находится в процессе разрешения.
>
> См. [`routeLoader$`](/docs/(qwikcity)/route-loader/index.mdx) для ранней выборки данных в рамках начального HTTP-запроса.
...
Обратный вызов, переданный в [`useResource$()`](/docs/(qwik)/components/state/index.mdx), запускается сразу после завершения обратного вызова [`useTask$()`](/docs/(qwik)/components/tasks/index.mdx#usetask). Более подробную информацию см. в разделе [Lifecycle](../tasks/index.mdx#lifecycle).
...
Qwik гарантирует, что всё состояние приложения всегда сериализуемо. Это важно для того, чтобы приложения Qwik имели свойство [возобновляемости](/docs/(qwik)/concepts/resumable/index.mdx).
