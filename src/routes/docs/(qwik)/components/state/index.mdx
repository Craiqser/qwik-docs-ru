  ...
  return (
    <>
      ...
      <button onClick$={() => (store.nested.fields.are = 'tracked')}>
        Клик по мне не работает
      </button>
      <br />
      <button onClick$={() => (store.nested = { fields: { are: 'tracked' } })}>
        Клик по мне работает
      </button>
    </>
  );
});
  ...
  return (
    <>
      ...
      <button onClick$={() => (store.nested.fields.are = 'отслеживается')}>
        Клик работает, так как изменение состояния отслеживается рекурсивно
      </button>
      <br />
      <button onClick$={() => (store.nested = { fields: { are: 'отслеживается' } })}>
        Клик всё ещё работает
      </button>
    </>
  );
});
...
export default component$(() => {
  ...
  // Предоставляем состояние контексту под идентификатором контекста
...
Вычисляемые свойства - это значения, которые получены из существующего состояния.
...
export default component$(() => {
  ...
  return (
    <>
      ...
      <div>Название: {name.value}</div>
      <div>Название строчными: {capitalizedName.value}</div>
    </>
  );
});
```
...
export default component$(() => {
  ...
  const prTitle = useResource$(async ({ track }) => {
    track(prNumber); // Требуется явное отслеживание
...
Благодаря фрагментарной реактивности Qwik, будут обновляться только те компоненты, которые зависят от состояния. Это большой прирост в производительности по двум причинам:

1. Меньше кода для выполнения означает, что обновления в приложении будут рендериться быстрее.
2. Часто код не нужно загружать при запуске приложения (или когда-либо).

## `noSerialize()`

Qwik гарантирует, что всё состояние приложения всегда сериализуемо. Это важно для того, чтобы приложения Qwik имели свойство [возобновляемости](/docs/(qwik)/concepts/resumable/index.mdx). Иногда необходимо хранить данные, которые нельзя сериализовать. Например, ссылку на стороннюю библиотеку, такую ​​как [редактор Monaco](https://microsoft.github.io/monaco-editor/). В такой ситуации используйте `noSerialize()`, чтобы пометить значение как несериализуемое.
