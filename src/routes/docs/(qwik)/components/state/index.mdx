...
export default component$(() => {
  ...
  return (
    <>
      ...
      <button onClick$={() => {
        // Несмотря на то, что мы мутируем вложенный объект, это вызовёт повторный рендер.
        store.nested.fields.are = 'tracked'
      }}>
        Клик работает, так как состояние просматривается на всю глубину
      </button>
      <br />
      <button onClick$={() => {
        // Поскольку состояние просматривается на всю глубину, это вызовет повторный рендер.
...
export default component$(() => {
  ...
  return (
    <>
      <button onClick$={() => state.increment()}>Прибаивть</button>
      <p>Счётчик: {state.count}</p>
    </>
  );
});
