...
Обратный вызов, переданный в [`useResource$()`](/docs/(qwik)/components/resource/index.mdx), запускается сразу после завершения обратного вызова [`useTask$()`](/docs/(qwik)/components/tasks/index.mdx#usetask). Более подробную информацию см. в разделе [Lifecycle](../tasks/index.mdx#lifecycle).
