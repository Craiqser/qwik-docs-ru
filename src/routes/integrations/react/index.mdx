...
> **Примечание:** Вы НЕ МОЖЕТЕ использовать компоненты React в Qwik без их предварительного преобразования с помощью `qwikify$()`. Несмотря на то, что [компоненты React и Qwik выглядят похоже](/docs/(qwik)/cheat/qwik-react/index.mdx), они принципиально очень разные.
