...
Действия объявляются с помощью функции `action$()` в любом месте. Действия похожи на [загрузчики](/docs/qwik-city/loader$), но они не выполняются во время рендеринга. Вместо этого они выполняются при явном вызове, то есть запускаются после некоторого взаимодействия с пользователем, например, при нажатии кнопки мыши или при отправке формы.
...
Действия также могут быть запущены из кода с помощью метода `action.run()`, т.е. вам не обязательно нужен компонент `<Form/>`. Вы можете запустить действие по нажатию кнопки или при наступлении любого другого события, точно так же, как вы бы сделали это функцией.
