...
Этот компонент обычно располагается как дочерний элемент `<body>`, в большинстве интеграций вы найдёте его в файле `src/root.tsx`.
...
export default component$(() => {
  return (
    <div>
      <a href="/docs" class="my-link">Полная перезагрузка страницы</a>
      <Link href="/docs" class="my-link">SPA-навигация</Link>
    </div>
  )
});
```
