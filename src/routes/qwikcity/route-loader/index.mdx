...
Вы не должны использовать загрузчик маршрута для создания REST API, для этого вам лучше использовать [конечную точку](/qwikcity/endpoints/index.mdx), которая позволяет вам иметь более полный контроль над заголовками и телом ответа.
