...
Класс `cookie` используется для получения и установки кук в вашем приложении Qwik City. Он предоставляет следующие методы:
...
Полученное значение - это простой объект с тремя полями:
  1. `value`: Содержит значение кук в виде строки;
