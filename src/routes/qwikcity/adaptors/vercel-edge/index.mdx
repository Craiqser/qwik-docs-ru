...
Кроме того, в `package.json` будет обновлен скрипт `build.server` для сборки Vercel Edge.
...
Обратите внимание, что для выполнения этого шага вам может понадобиться [учётная запись Vercel](https://docs.Vercel.com/get-started/)!

## Развёртывание в производственной среде

После установки интеграции с помощью `npm run qwik add` проект готов к развёртыванию в Vercel. Однако вам нужно будет создать репозиторий git и отправить в него код.
